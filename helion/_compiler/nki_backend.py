...
This module contains the NKI op-lowering table (``NKIOpOverrides``) and the
``NKIBackend`` codegen backend, extracted from the historical monolithic
``backend.py``. Importing this module registers ``NKIBackend`` with the
backend registry (see the ``register_compiler_backend`` call at the bottom),
so it is loaded lazily by ``backend_registry._maybe_register_nki``.

All heavy dependencies (CompileEnvironment, device_function, torch_xla, etc.)
are imported lazily inside method bodies to preserve plugin semantics: this
module must import cleanly even where NKI cannot actually run.
"""
...
class NKIBackend(Backend):
    ...
    def pad_factory_tensors_to_power_of_2(self) -> bool:
        # NKI requires exact tile extents; padding factory tensors to a
        # power of two would break the partition/free DMA shape match.
        # This replaces the reference branch's patch_tensor_factories guard
        # (which upstream moved onto this backend property).
        return False

    def static_rdim_size(self, numel: int) -> int:
        # NKI reduction DMA paths require source/destination extents to
        # match; keep the persistent-reduction extent exact (no power-of-2
        # rounding). Replaces the reference's inline ReductionLoopBlockSizeSource
        # guard.
        return numel
...
# Register this backend with the registry (triggered by
# backend_registry._maybe_register_nki importing this module).
