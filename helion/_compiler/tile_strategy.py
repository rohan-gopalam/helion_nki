            ...
            for i, block_idx in enumerate(block_ids):
                ...
                if isinstance(bs_info.size, (int, torch.SymInt)):
                    ...
                    if env.is_jagged_tile(block_idx):
                        _all_jagged = env.jagged_tile_parent_ids
                        _is_outer_jagged = any(
                            other_bid > block_idx
                            for other_bid in _all_jagged
                            if other_bid != block_idx
                        )
                    if _is_outer_jagged:
                        # Use affine_range: compute static upper bound as the
                        # BlockSizeSpec max_size (a power-of-two ceiling on the
                        # hint).  The per-row jagged mask generated by
                        # _NKINDTileStrategy._setup_mask zeroes out positions
                        # that exceed each row's true feature count.
                        _block_size_int = (
                            int(block_sizes[i])
                            if isinstance(block_sizes[i], (int, bool))
                            else 1
                        )
                        try:
                            _bs_spec = env.config_spec.block_sizes.block_id_lookup(block_idx)
                            _static_end = _bs_spec.max_size
                        except Exception:
                            _static_end = _block_size_int
                        ends[i] = _static_end
                        proxy_ends[i] = _static_end
                        # Do NOT add to _nki_dyn_loops — offset_var is a plain
                        # Python int from affine_range, so downstream codegen
                        # (iota, tensor_scalar, DMA AP) can use it directly.
                        continue
